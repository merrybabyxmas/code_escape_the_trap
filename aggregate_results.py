...
PER_SAMPLE_PATH = os.path.join(OUTPUT_DIR, "per_sample_results_full.json")
# ---------------------
...
def main():
    ...
    for model_name, scenarios in data.items():
        ...
        for sid, entry in scenarios.items():
            m = entry['metrics']
            target = metrics_a if "set_a" in sid else metrics_b
    ...
    print(f"✅ Finalized aggregation for {len(summary_a)} models in Set A and {len(summary_b)} models in Set B.")
